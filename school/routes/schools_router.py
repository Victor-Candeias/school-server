...
@schools_router.get("/find")
async def find_school(request: Request):
    return await utilities.get_documents(api_client=api_client, endpoint="find", request=request, collection=SCHOOLS_COLLECTION, source="students_routes", method="find_class_students")
...
@schools_router.get("/findbyid")
async def find_school_byid(request: Request):
    return await utilities.get_documents(api_client=api_client,endpoint ="findbyid", request=request, collection=SCHOOLS_COLLECTION, source="students_routes", method="findbyid_class_students")

# curl -X POST http://127.0.0.1:8001/schools/add -H  "Content-Type: application/json" -d "{ \"userid\": \"67e32c8bf97d9bb2e993e50d\", \"classid\": \"67e32c8bf97d9bb2e993e50d\", \"id\": \"1\", \"name\": \"nome aluno\", \"email\": \"aluno@ctt.pt\" }"
@schools_router.post("/add")
async def add_school(request: Request):
    return await utilities.add_document(api_client=api_client, request=request, collection=SCHOOLS_COLLECTION, source="students_routes", method="add_class_student")
